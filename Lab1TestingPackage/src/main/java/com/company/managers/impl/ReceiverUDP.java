...
import com.company.network.udp.StoreServerUDP;
...
public class ReceiverUDP implements IReceiver {
    ...
    private static BlockingQueue<DatagramPacket> queue = new ArrayBlockingQueue<>(16);

    public ReceiverUDP(DatagramPacket datagramPacket) throws InterruptedException {
    ...
    public void receiveMessage() {
        new Thread(){
            ...
            public void run(){
                ...
                Message m = processor.process(Decriptor.getInstance().decryptAndProcess(Arrays.copyOfRange(data, ProtocolInfo.O_MESSAGE,
                        ProtocolInfo.O_MESSAGE + messageLength)));
                m.setToEncrypt(false);
                try {
                    Sender sender = new Sender(m.getBytes());
                    DatagramPacket info = queue.take();
                    sender.send(info.getAddress(), info.getPort());
